    ...
    switch(uart->msg_state) {
        case(STATESTART):                                   //Have yet to receive a valid SOF
            ...
            if(uart->rxbuf[uart->consumer_index] == RXSOF) {
                ...
                ResetRxBuffer(& uart);                            //Reset this because messaging handling didn't occur while calibrating
                MainMenu(& uart);                         //Run the calibration routine
                uart->inmenu = false;                     //Clear this now that the sensors have been calibrated
            }
            else {
                IncrementConsumer(& uart);                        //Not a valid byte, keep looking
            }
        break;
